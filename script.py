...
def send_signal(signal):
    url = 'http://localhost:5000/postsignal'
    data = {'signal': signal}
    try:
        response = requests.post(url, json=data)
        print(f"Signal sent. Server response: {response.text}")
    except Exception as e:
        print(f"Failed to send signal: {e}")

def get_signal():
    url = 'http://localhost:5000/getsignal'
    try:
    ...
    with Client(TOKEN) as client:
        while True:
            ...
            if not data.empty:
                ...
                data.loc[(data['fast_sma'].shift(1) > data['slow_sma'].shift(1)) & (data['fast_sma'] > data['slow_sma']), 'signal'] = 'В лонге'
                data.loc[(data['fast_sma'].shift(1) < data['slow_sma'].shift(1)) & (data['fast_sma'] < data['slow_sma']), 'signal'] = 'В шорте'
                data.loc[(data['fast_sma'].shift(1) < data['slow_sma'].shift(1)) & (data['fast_sma'] > data['slow_sma']), 'signal'] = 'Вошли в лонг'
                data.loc[(data['fast_sma'].shift(1) > data['slow_sma'].shift(1)) & (data['fast_sma'] < data['slow_sma']), 'signal'] = 'Вошли в шорт'

                # Проверка на сигналы и отправка
                last_signal = data.iloc[-1]['signal']
                current_flag = get_signal()
